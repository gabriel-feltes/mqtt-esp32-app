...
function setupEventListeners() {
  ...
  // Logar todas as mensagens, exceto 'heartbeat'
  if (msg.toLowerCase() !== 'heartbeat') {
    fetch('https://back-end-gabriel-feltes-dos-santos-projects.vercel.app/log', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        topic,
        message: msg,
        user: localStorage.getItem('mqtt_username') || 'anon'
      })
    }).catch(err => {
      console.error('Erro ao enviar log MQTT para o backend:', err)
    })
  }
})
